...
class RosTasksRunner(tasks_runner.TasksRunner):
    def __init__(self, tasks, name, *args, **kwargs):
        self.name = name
        self.debug_topics = {}
        self.register_debug_topics(
            [(name + '/' + topic[0], topic[1]) for topic in ('task_ix', 'Int16'), ('active_task_kind', 'String'),])
        ...
        super(RosTasksRunner, self).__init__(tasks, *args, **kwargs)
    ...
    def register_debug_topics(self, topics):
        ...
        *topics* should be a list of pairs, (topic_name, data_type), e.g.:
        ...
        for (topic, datatype_s) in topics:
            if (topic in self.debug_topics) and (self.debug_topics[topic][0] == datatype_s):
